...
def brainf_string(arg):
    try:
        ...
        while i < len(arg):
            # if turing_machine[pointer] == None:
            #     turing_machine[pointer] = 0
...
class BrainFCog (commands.Cog):
    ...
    async def bf(self, ctx, *, arg):
        print(arg)
        await ctx.reply('`' + brainf_string(arg) + '`')

    @bf.error
    async def bf_error(self, ctx, error):
        if isinstance(error, commands.MissingRequiredArgument):
            await ctx.send('$bf (brainf code)')

        if isinstance(error, MemoryError):
            await ctx.send('*`Failed to compile: Memory Error`*')
        if isinstance(error, ValueError):
            await ctx.send('*`Failed to compile: Value Error`*')
